...
public abstract class JMXServerConnection implements ServerConnection {
	...
	public Set<ObjectName> queryNames(ObjectName objectName) {
		...
		Set mbeans = (Set) callMBeanServer("queryNames", methodSignature, methodArgs);
		return toJmanageObjectNameInstance(mbeans);
	}

	/**
	 * Invokes the given "operationName" on the object identified by "objectName".
	 * 
	 * @param objectName
	 * @param operationName
	 * @param params
	 ...
	 * @return
	 */
	public Object invoke(ObjectName objectName, String operationName, Object[] params,
			String[] signature) {

		Class[] methodSignature = new Class[] { javax.management.ObjectName.class,
				String.class, new Object[0].getClass(), new String[0].getClass() };
		Object[] methodArgs = new Object[] { toJMXObjectName(objectName), operationName,
				params, signature };
	...
	public ObjectInfo getObjectInfo(ObjectName objectName) {
		...
		MBeanInfo mbeanInfo = (MBeanInfo) callMBeanServer("getMBeanInfo", methodSignature,
				methodArgs);
	...
	/**
	 * Returns a list of ObjectAttribute objects containing attribute names and values for
	 * the given attributeNames
	 * 
	 * @param objectName
	 * @param attributeNames
	 * @return
	 */
	public List<ObjectAttribute> getAttributes(ObjectName objectName,
	 ...
	 * @param objectName
	 * @param attributeList
	 *          list of ObjectAttribute objects
	 */
	...
	protected Map<ObjectNotificationListener, Object> notifications = new HashMap<ObjectNotificationListener, Object>();
	protected Map<ObjectNotificationFilter, NotificationFilter> notifFilters = new HashMap<ObjectNotificationFilter, NotificationFilter>();

	public void addNotificationListener(ObjectName objectName,
			ObjectNotificationListener listener, ObjectNotificationFilter filter,
			Object handback) {
		...
		Class[] methodSignature = new Class[] { javax.management.ObjectName.class,
				NotificationListener.class, NotificationFilter.class, Object.class };
		Object[] methodArgs = new Object[] { toJMXObjectName(objectName), notifListener,
				notifFilter, handback };
	...
	public void removeNotificationListener(ObjectName objectName,
		...
		NotificationListener notifListener = (NotificationListener) notifications
				.remove(listener);
	...
	public void createMBean(String className, ObjectName name, Object[] params,
			String[] signature) {
		Class[] methodSignature = new Class[] { String.class,
				javax.management.ObjectName.class, new Object[0].getClass(),
				new String[0].getClass() };
		Object[] methodArgs = new Object[] { className, toJMXObjectName(name), params,
				signature };
	...
	/**
	 * checks if this connection is open. It simply calls getMBeanCount() method on
	 * MBeanServer Connection. If there is no errors, then the connection is assumed to be
	 * open.
	 * 
	...
	public boolean isOpen() {
		try {
			...
			logger.log(Level.INFO, "Connection to the server is lost. " + "Error message: "
					+ e.getMessage());
			return false;
		}
	}
	...
	protected static ObjectInfo toObjectInfo(ObjectName objectName, MBeanInfo mbeanInfo) {
		...
		ObjectConstructorInfo[] constructors = toObjectConstructors(mbeanInfo
				.getConstructors());
		ObjectOperationInfo[] operations = toObjectOperations(mbeanInfo.getOperations());
		ObjectNotificationInfo[] notifications = toObjectNotifications(mbeanInfo
				.getNotifications());
		return new ObjectInfo(objectName, mbeanInfo.getClassName(), mbeanInfo
				.getDescription(), attributes, constructors, operations, notifications);
	}

	protected static ObjectAttributeInfo[] toObjectAttributes(
	...
	protected static ObjectAttributeInfo toObjectAttributeInfo(MBeanAttributeInfo attribute) {
		...
		return new ObjectAttributeInfo(attribute.getName(), attribute.getDescription(),
				attribute.getType(), attribute.isWritable(), attribute.isReadable(), attribute
						.isIs());
	}

	protected static ObjectConstructorInfo[] toObjectConstructors(
	...
	protected static ObjectConstructorInfo toObjectConstructorInfo(
			MBeanConstructorInfo constructor) {
		return new ObjectConstructorInfo(constructor.getName(), constructor.getDescription(),
				toObjectParameters(constructor.getSignature()));
	}

	protected static ObjectOperationInfo[] toObjectOperations(
	...
	protected static ObjectOperationInfo toObjectOperationInfo(MBeanOperationInfo operation) {
		return new ObjectOperationInfo(operation.getName(), operation.getDescription(),
				toObjectParameters(operation.getSignature()), operation.getReturnType(),
				operation.getImpact());
	}

	protected static ObjectNotificationInfo[] toObjectNotifications(
	...
	protected static ObjectNotificationInfo toObjectNotificationInfo(
			MBeanNotificationInfo notification) {
		return new ObjectNotificationInfo(notification.getNotifTypes(), notification
				.getName(), notification.getDescription());
	}

	protected static ObjectParameterInfo[] toObjectParameters(
	...
	protected static ObjectParameterInfo toObjectParameterInfo(MBeanParameterInfo parameter) {
		...
		return new ObjectParameterInfo(parameter.getName(), parameter.getDescription(),
				parameter.getType());
	}

	protected static List<ObjectAttribute> toObjectAttributeList(AttributeList attrList) {
		final List<ObjectAttribute> objAttrList = new ArrayList<ObjectAttribute>(attrList
				.size());
	...
	protected static ObjectNotification toObjectNotification(Notification n) {
		...
		return new ObjectNotification(n.getType(), n.getSource(), n.getSequenceNumber(), n
				.getTimeStamp(), n.getMessage(), n.getUserData());
	}
