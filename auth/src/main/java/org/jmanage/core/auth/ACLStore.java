 ...
 */
public class ACLStore implements ACLStoreMBean {
	...
	private Map<String, ACL> aclNameToACLMap = Collections
			.synchronizedMap(new HashMap<String, ACL>());
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.jmanage.core.auth.ACLStoreMBean#reloadConfig()
	 */
	public void reloadConfig() {
		synchronized (aclNameToACLMap) {
			aclNameToACLMap.clear();
			load();
			logger.info("Reloaded ACLs");
		}
	}

	private void load() {
		final String configFile = CoreUtils.getConfigDir() + File.separator + ACL_CONFIG_FILE;
		try {
	...
	private void storeACL(String aclName, String aclContext, String authorizedList) {
		...
		logger.fine("Added ACL: " + aclName + " - " + aclContext + " - " + authorizedList);
	}
}