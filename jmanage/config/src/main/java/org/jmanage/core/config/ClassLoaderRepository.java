...
public class ClassLoaderRepository {
    ...
    public static ClassLoader getClassLoader(URL[] classpath,
        ...
        if(classLoader == null){
            logger.fine("Creating new ClassLoader for classpath : " +
                    StringUtils.toString(classpath) + ", isCompatibleJMX=" +
                    isCompatibleJMX);
            if(!isCompatibleJMX){
    ...
    private static class Key {
        ...
        public boolean equals(Object obj){
            if(obj instanceof Key){
                Key key = (Key)obj;
                if(key.isCompatibleJMX == this.isCompatibleJMX
                        && compare(key.classpath,  this.classpath)){
                    return true;
                }
            }
            return false;
        ...
        public int hashCode(){
            return classpath.hashCode() * 31 +
                    Boolean.valueOf(isCompatibleJMX).hashCode();
        }
